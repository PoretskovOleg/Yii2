...
class CommercialProposalEmail extends Model
{
    public $email;
    public $subject;
    public $text;
    public $attachment;
    public $commercial_proposal_id;
    public $commercial_proposal_filename;

    public function rules()
    {
        return [
            [['email', 'subject', 'text', 'commercial_proposal_filename'], 'required'],
            [['email'], 'email'],
            [['email', 'subject', 'commercial_proposal_filename'], 'string', 'max' => 255],
            [['text'], 'string'],
    ...
    public function attributeLabels() {
        return [
            ...
            'subject' => 'Тема',
            'text' => 'Текст',
            'commercial_proposal_filename' => 'Имя файла',
    ...
    {
        $commercial_proposal = CommercialProposal::findOne(\Yii::$app->request->post('commercial_proposal_id'));
        $commercial_proposal_attachments = $commercial_proposal->files;
        ...
        try {
            $message = $mailer->compose()
                ...
                ->attachContent($pdf, ['fileName' => $this->commercial_proposal_filename, 'contentType' => 'application/pdf']);

            foreach ($commercial_proposal_attachments as $attachment) {
                $message->attach(\Yii::getAlias('@webroot') . \Yii::getAlias('@commercial_proposals_attachments/') . $attachment->filename);
            }

            $this->attachment = UploadedFile::getInstance($this, 'attachment');
            if (!empty($this->attachment)) {
                $path = \Yii::getAlias('@webroot') . \Yii::getAlias('@commercial_proposals_temp');
