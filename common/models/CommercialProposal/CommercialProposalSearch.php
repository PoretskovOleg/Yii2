...
class CommercialProposalSearch extends Model
{
    public $id;
    public $order_id;
    ...
    public function search($params)
    {
        ...
        $query = CommercialProposal::find()
            ->from('commercial_proposals cp')
            ->join('LEFT JOIN', $old_db_name . '.contractor contractor', 'contractor.contractor_id = cp.contractor_id');
        ...
        if (!empty($this->total)) {
            $query->andWhere(['cp.total' => $this->total]);
        }

        if (!empty($this->organization_id)) {
            $query->andWhere(['cp.organization_id' => $this->organization_id]);
        }

        if (!empty($this->subject_id)) {
            $query->andWhere(['cp.subject_id' => $this->subject_id]);
        }

        if (!empty($this->status_id)) {
            $query->andWhere(['cp.status_id' => $this->status_id]);
        }

        if (!empty($this->primary)) {
            $query->andWhere(['cp.primary' => $this->primary]);
        }

        if (!empty($this->manager_id)) {
            $query->andWhere(['cp.manager_id' => $this->manager_id]);
        }
