...
use common\models\Invoice\InvoiceEmail;
use common\models\Invoice\InvoiceGood;
use common\models\Invoice\InvoiceTemplate;
use common\models\Invoice\InvoiceComment;
use common\models\Invoice\InvoiceFile;
...
class InvoiceController extends Controller
{
    ...
    public static function getUserRules() {
        return [
            ['id' => 1, 'name' => 'Просматривать реестр счетов'],
            ['id' => 2, 'name' => 'Создавать/редактировать счета'],
            ['id' => 3, 'name' => 'Менять данные шаблона в счёте'],
            ['id' => 4, 'name' => 'Удалять счета'],
            ['id' => 5, 'name' => 'Редактировать счёт после отправки'],
        ];
    }

    public function actionDeleteFile() {
        $file_id = Yii::$app->request->post('key');
        $result = InvoiceFile::deleteAll(['id' => $file_id]);
    ...
    public function actionUploadImage() {
        ...
        if ($image) {
            $directory = Yii::getAlias('@invoices_images');
    ...
    public function actionCreate($instance_id = false)
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if (!Invoice::findOne(['order_id' => $model->order_id])) {
                $model->primary = true;
            }
        ...
        if ($instance_id) {
            ...
            $model->status_id = Invoice::STATUS_NEW;
            $model->primary = false;
    ...
    public function actionGetTemplateData($template_id) {
        $template = InvoiceTemplate::findOne($template_id);
        if ($template) {
            $signers = [];
    ...
    public function actionAddComment() {
        $model = new InvoiceComment();
        $model->load(Yii::$app->request->post());
        $cp_id = $model->invoice_id;

        $success = false;
        if ($model->save()) {
            $success = true;
            $model = new InvoiceComment();
            $model->invoice_id = $cp_id;
        }
    ...
    public function actionGetHtml($id) {
        ...
        $html = '<style>' . file_get_contents(Yii::getAlias('@webroot/css/invoice/pdf.css')) . '</style>';
        //$html = '';
        $html .= $this->renderPartial('export/_header', $templateData);
        $html .= $this->renderPartial('export/_goods_list', $templateData);
        $html .= $this->renderPartial('export/_goods_total', $templateData);
        $html .= $this->renderPartial('export/_standart_note', $templateData);
        $html .= $this->renderPartial('export/_note', $templateData);
        $html .= $this->renderPartial('export/_signs', $templateData);
        $html .= $this->renderPartial('export/_footer', $templateData);
    ...
    public function actionGetPdf($id, $output = 'I')
    {
        ...
        $mpdf_buffer = new \mPDF('', [210, 5 * $mpdf->h],10,'arial', 15,15, 0, 0, 0, 0);
