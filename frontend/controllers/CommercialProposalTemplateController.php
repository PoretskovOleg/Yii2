...
class CommercialProposalTemplateController extends Controller
{
    const MODULE_NAME = 'commercial-proposal-templates';

    public static function getUserRules() {
        return [
            ['id' => 1, 'name' => 'Просматривать реестр шаблонов коммерческих'],
            ['id' => 2, 'name' => 'Создавать/редактировать шаблоны коммерческих'],
            ['id' => 2, 'name' => 'Удалять шаблоны коммерческих'],
        ];
    }
    ...
    public function actionIndex()
    {
        $dataProvider = new ActiveDataProvider([
            'query' => CommercialProposalTemplate::find(),
            'sort' => false,
        ]);
    ...
    {
        $model = new CommercialProposalTemplate();
    ...
    public function actionGetPdf($id)
    {
        ...
        $mpdf->setTitle('Шаблон коммерческого предложения');
        $mpdf->img_dpi = 72;

        $stylesheet = file_get_contents(Yii::getAlias('@webroot/css/commercial-proposal-template/pdf.css'));
    ...
    {
        if (($model = CommercialProposalTemplate::findOne($id)) !== null) {
            return $model;
        } else {
