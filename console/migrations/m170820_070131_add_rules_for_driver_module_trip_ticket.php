...
class m170820_070131_add_rules_for_driver_module_trip_ticket extends Migration
{
    public function safeUp()
    {
        //название модуля для отображения в старой админке
        $module_title = 'Модуль Водитель';
        //название модуля для использования в checkRule($module_name, $rule_id)
        $module_name = frontend\controllers\DriverTripTicketsController::MODULE_NAME;
        //массив правил [['id' => '1', 'name' => 'Правило 1'], ['id' => '2', 'name'...
        $rules = frontend\controllers\DriverTripTicketsController::getUserRules();
    ...
    public function safeDown()
    {
        ...
        $old_rule_ids = Yii::$app->db->createCommand('
                SELECT old_rule_id FROM rules WHERE module = :module AND rule_id IN (:rule_id)
            ', ['module' => $module_name, 'rule_id' => [7, 8, 9, 10]])->queryColumn();

        if (!is_array($old_rule_ids)) {
            return false;
        }

        Yii::$app->db->createCommand()->delete('rules', ['and', ['=', 'module', $module_name], ['in', 'rule_id', [7, 8, 9, 10]]])->execute();
